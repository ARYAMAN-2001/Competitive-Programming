...
// }
ll a[1001][1001],i,n,k,m,x,y,p,q,j,cnt,b,c,dp[1001][1001];
vector<pll> ans;
void check(ll v)
{
 for(b=n;b>=1;b--)
 {
    for(c=m;c>=1;c--)
    {
        ...
        {
            if(a[b][c]==a[b+1][c] and a[b][c]==a[b][c+1] and a[b][c]==a[b+1][c+1])
                dp[b][c]=min3(dp[b+1][c],dp[b][c+1],dp[b+1][c+1])+1;
            else 
                dp[b][c]=1;
        ...
        check(1);
                 ...
                 if(ans.size()==0)   
                    cout<<"Draw"<<endl;
                      ...
                      ll d=-999999999,tot=99999999999;
                       fo(i,0,ans.size())
                       {
                        for(p=ans[i].first;p<ans[i].first+k;p++)
                        {
                            for(q=ans[i].second;q<ans[i].second+k;q++)
                            {
                                // cout<<p<<" "<<q<<" "<<m1[{p,q}]<<endl;
                                d=max(d,val[p][q]);

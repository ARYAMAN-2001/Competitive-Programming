...
void solve(vll &v,ll d) {
    if(v.size()<=1) 
    return;
    if(v.size()==2){
        if(!query2(v[0],v[1])) swap(v[0],v[1]);
        return;
    }
    vll p1,p2;
    p1.pb(v[0]);
    fo(i,1,v.size()) {
        if(query1(v[0],v[i],d)) p1.pb(v[i]);
        else p2.pb(v[i]);
    }
    solve(p1,2*d);
    solve(p2,2*d);

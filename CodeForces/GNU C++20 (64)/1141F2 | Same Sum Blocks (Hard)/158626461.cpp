...
int main() {
    ...
    while (s.size())
    {
        ll x = *s.begin(), idx = 0, ans = -1, y = m[x].size();
        vll val(y);
        vector<bool> vis(y);
        // cout << y << " " << x << " " << tot << endl;
        if (m[x].size() <= tot)
    ...
    cout << ans1[num].size() << endl;
    fo(i, 0, ans1[num].size())
    cout << ans1[num][i] / d << " " << ans1[num][i] % d << endl;

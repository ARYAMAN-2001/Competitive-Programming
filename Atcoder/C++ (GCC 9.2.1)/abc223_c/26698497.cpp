...
int main() {
    ...
    vector<double> dp(n + 1), dp1(n + 1);
    fo(i, 0, n)
    {
        dp[i + 1] = dp[i] + double(a[i]) / double(b[i]);
    }
    for (i = n - 1; i >= 0; i--)
    {
        dp1[i] = dp1[i + 1] + double(a[i]) / double(b[i]);
    }
    double t1, t2;
    fo(i, 0, n)
    {
    ...
    double ans = double(val[idx]) + double(a[idx]) / 2.0;
    ans += (t2 - t1) / double(2) * double(b[idx]);

...
int main() {
    ...
    fo(i, 0, n)
    {
        fo(j, 0, m)
        {
            if (j)
            {
                dp[i + 1][(j + 1) % m] = max(dp[i + 1][(j + 1) % m], dp[i][j] + a[i]);
            }
            else
            {
                dp[i + 1][(j + 1) % m] = max(dp[i + 1][(j + 1) % m], dp[i][j] + a[i] - k);
            }
            dp[i + 1][1 % m] = max(dp[i + 1][1 % m], a[i] - k);
            // cout << dp[i + 1][j + 1] << " " << i << " " << j << endl;
            ans = max(ans, dp[i + 1][(j + 1) % m]);
        }
    }
    cout << ans << endl;

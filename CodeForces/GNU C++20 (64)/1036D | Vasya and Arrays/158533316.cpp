...
int main() {
    ...
    fo(i, 0, m)
    cin >> b[i];
    ll l = 0, r = 0, sum1 = 0, sum2 = 0, ans = 0;
    while (l < n or r < m)
    {
        while (r != m and (sum1 > sum2 or sum2 == 0))
        {
            sum2 += b[r];
            r++;
        }
        while (l != n and (sum1 < sum2 or sum1 == 0))
        {
            sum1 += a[l];
            l++;
        }
        ...
        if(l==n and sum2>sum1)
            break;
        else if(r==m and sum1>sum2)
            break;
        // cout << sum1 << " " << sum2 << " " << l << " " << r << endl;
    }
    // cout << l << " " << r << " " << sum1 << " " << sum2 << endl;
    if (l != n or r != m or sum1 or sum2)
        cout << -1 << endl;
    else
        cout << ans << endl;

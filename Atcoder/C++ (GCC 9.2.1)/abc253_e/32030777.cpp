...
int main() {
    ...
    fo(i, 2, n + 1)
    {
        fo(j, 1, m + 1)
        {
            ...
            {
                dp[j][i] += (dp1[max(j - k, 0ll)][i - 1]) % mod1;
                dp[j][i] %= mod1;
                if (dp[j][i] < 0)
                    dp[j][i] = (dp[j][i] % mod1 + mod1) % mod1;
                dp[j][i] += ((dp1[m][i - 1] % mod1 - dp1[min(m, j + k - 1)][i - 1] % mod1) % mod1 + mod1) % mod1;
                dp[j][i] %= mod1;
                if (dp[j][i] < 0)
                    dp[j][i] += mod1;
            }
            // cout << dp[j][i] << " " << j << " " << i << endl;
            dp1[j][i] = (dp1[j - 1][i] + dp[j][i]) % mod1;
        }
    }

...
int main() {
    ...
    fo(i, 0, n)
    {
        ...
        {
            ll res = 0;
            while (res == 0 and (i + cnt)<n and s[i]>t[i + cnt])
            {
                ll x = lb(all(v[t[i + cnt] - 'a']), last) - v[t[i + cnt] - 'a'].begin();
                // cout << x << " " << t[i] << endl;
                if (x and v[t[i + cnt] - 'a'][x - 1] > i)
                {
                    res = 1;
                    ans.pb(i);
                    ans.pb(v[t[i + cnt] - 'a'][x - 1]);
                    last = v[t[i + cnt] - 'a'][x - 1];
                }
                else
                    cnt++;
            }

...
int main() {
    ...
    while (t--)
    {
        ...
        while (1)
        {
            ...
            {
                ll idx1 = lb(all(v1), idx) - v1.begin();
                if (idx1 == 0)
                    break;
                else
                {
        ...
        if ((a[0] + a[n - 1]) % 2)
        {
            ...
            fo(i, 0, n)
            {
                if ((a[i] + a[0] + 1) % 2)
                {
                    if (a[n - 1] % 2)
                    {   
                        ll idx1 = ub(all(v), i) - v.begin();
                        // cout<<ans<<" "<<v[idx1]<<" "<endl;
                        ans1 = min(ans1, ans + 1 + val[v[idx1]]);
                    }
                    else
                    {
                        ll idx1 = ub(all(v1), i) - v1.begin();
                        // cout<<ans<<" "<<v1[idx1]<<endl;
                        ans1 = min(ans1, ans + 1 + val[v1[idx1]]);
                    }
                    ans++;
                }
            }

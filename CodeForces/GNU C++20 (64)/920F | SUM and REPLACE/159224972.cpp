...
int main() {
    ...
    while (q--)
    {
        ...
        {
            auto it = s.lb(y);
            while (it != s.end() and * it <= z)
            {
                ll val = *it;
                it++;
                // cout << val << " " << a[val] << " % " << dp[a[val]] << endl;
                a[val] = dp[a[val]];
                update(1, 0, n - 1, val, a[val]);
                if (a[val] == 2)
                    s.erase(val);
            }
        }

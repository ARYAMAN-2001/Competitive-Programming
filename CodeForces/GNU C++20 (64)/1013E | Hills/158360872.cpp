...
int main() {
    ...
    ll n;
    cin >> n;
    ll a[n + 1], i, j, dp[n + 1][n / 2 + 10], dp1[n][n / 2 + 1];
    fo(i, 0, n)
    {
        cin >> a[i];
        fo(j, 0, n / 2 + 10)
    ...
    dp1[1][1] = min(a[1], a[0] - 1);
    dp[1][1] = max(0ll, a[0] - a[1] + 1) + max(0ll, a[2] - a[1] + 1);
    if (n != 1 and dp[1][1] < dp[0][1])
        dp1[2][1] = min(a[2], a[1] - 1);
    dp[1][1] = min(dp[1][1], dp[0][1]);
    fo(i, 1, (n + 1) / 2 + 1)
    {
        fo(j, 2, n)
        {
            dp[j][i] = min3(dp[j][i], dp[j - 1][i], dp[j - 2][i]);
            ll ans = 0;
            if (j)
            {
                ll x = dp1[j - 1][i - 1];
            ...
            if (j != n - 1)
            {
                ans += max(0ll, a[j + 1] - a[j] + 1);
                if (dp[j - 2][i - 1] + ans < dp[j][i])
                    dp1[j + 1][i] = min(a[j + 1], a[j] - 1);
            }
            dp[j][i] = min(dp[j - 2][i - 1] + ans, dp[j][i]);
    ...
    fo(i, 1, (n + 1) / 2 + 1)
    {
        fo(j, 2, n)
        {
            ...
            dp[j][i] = min(dp[j - 2][i - 1] + ans, dp[j][i]);
            // cout << dp[j][i] << " " << j << " " << i << " " << dp[j - 2][i - 1] << endl;
        }
    }
    fo(i, 1, (n + 1) / 2 + 1)
    {

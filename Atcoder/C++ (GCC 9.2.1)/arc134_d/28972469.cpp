...
int main() {
    ...
    fo(i, 0, n)
    {
        v.pb({a[i], a[i + n] * 1000000 + i});
        v1.pb({a[i], i});
    }
    sort(all(v));
    sort(all(v1));
    vll ans;
    if (a[v[0].second % d + n] < a[v[0].second % d])
    {
        idx = v[0].second % d;
        ans.pb(a[idx]);
        ans.pb(a[idx + n]);
        ...
        fo(i, 1, n)
        {
            ...
            if (v[i].first < ans[1] and v[i].second % d > idx)
            {
                sz++;
                ans.pb(v[i].first);
                ans.pb(a[v[i].second % d + n]);
                idx = v[i].second % d;
                if (a[v[i].second % d + n] != ans[1] and res == 0)
                {
                    max1 = a[v[i].second % d + n];
                    res = 1;
                }
            }
            else if (v[i].first == ans[1] and v[i].second % d > idx and v[i].first < max1 and res == 1)
            {
                sz++;
                ans.pb(v[i].first);
                ans.pb(a[v[i].second % d + n]);
    ...
    else
    {
        ...
        fo(i, 1, n)
        {
            ...
            if (v1[i].first < ans[1] and v1[i].second % d > idx)
            {
                sz++;
                ans.pb(v1[i].first);
                ans.pb(a[v1[i].second % d + n]);
                idx = v1[i].second % d;
                if (a[v1[i].second % d + n] != ans[1] and res == 0)
                {
                    max1 = a[v1[i].second % d + n];
                    res = 1;
                }
            }
            else if (v1[i].first == ans[1] and v1[i].second % d > idx and v1[i].first < max1 and res == 1)
            {
                sz++;
                ans.pb(v1[i].first);
                ans.pb(a[v1[i].second % d + n]);

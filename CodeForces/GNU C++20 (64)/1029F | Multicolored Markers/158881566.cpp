...
int main() {
    ...
    for (i = 1; i * i <= (n + m); i++)
    {
        if ((n + m) % i == 0)
        {
            ll x = i, y = (n + m) / i;
            // cout << x << " " << y << " " << ans << endl;
            ll idx = ub(all(v), x) - v.begin() - 1;
            // cout << idx << " " << v[idx] << endl;
            if (idx >= 0 and n / v[idx] <= y)
            {
                // cout << ans << " " << i << endl;
                ans = min(ans, 2 * i + 2 * (n + m) / i);
            }
            idx = ub(all(v), y) - v.begin() - 1;
            if (idx >= 0 and n / v[idx] <= x)
            {
                // cout << ans << " " << i << endl;
                ans = min(ans, 2 * i + 2 * (n + m) / i);
            }
            idx = ub(all(v1), x) - v1.begin() - 1;
            if (idx >= 0 and m / v1[idx] <= y)
            {
                // cout << ans << " " << i << endl;
                ans = min(ans, (2 * i + 2 * (n + m) / i));
            }
            idx = ub(all(v1), y) - v1.begin() - 1;
            if (idx >= 0 and m / v1[idx] <= x)
            {
                // cout << ans << " " << i << endl;
                ans = min(ans, (2 * i + 2 * (n + m) / i));
            }

...
int main() {
    ...
    while (l1 + 1 < r1)
    {
        ll mid = (l1 + r1) / 2, sum = 0, sum1 = 0, res = 0;
        fo(i, 0, k)
        {
            ll x = v[i].second % d1, y = k;
            if (i != k - 1)
                y = v[i + 1].second % d1;

            if (d[x] <= a[mid - 1] and res == 0)
                sum1 = min(m + 1, l[y] - 1);
            if (d[x] > a[mid - 1])
            {
                sum = max(sum, 2 * r[x]);
        ...
        ll ans = 1e18, max1 = sum / 2, max2 = 0, ans1 = 0, prev = l[0] - 1;
        ans = min(ans, sum - 2 * sum1 + m + 1);
        // cout << ans << " " << mid << endl;
        fo(i, 0, k)
        {
            ll x = v[i].second % d1, y = k;
            if (i != k - 1)
                y = v[i + 1].second % d1;
            if (d[x] <= a[mid - 1] or (d[x] > a[mid - 1] and max2 >= r[x]))
            {
                continue;
            }
            else
            {
                ans1 += 2 * (r[x] - max(max2 + 1, l[x]) + 1);
                max2 = max(max2, r[x]);
            }
            // cout << ans1 << " " << i << endl;
        }

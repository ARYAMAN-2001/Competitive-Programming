...
ll countDer(ll n)
{
    ...
    return (n - 1) * (countDer(n - 1) % mod + countDer(n - 2) % mod) % mod;
}
int main() {
    ...
    while (t--)
    {
        ...
        fo(i, 1, n + 1)
        {
            if (s.find(i) == s.end() and s1.find(i) == s1.end())
                cnt++;
            else if (s.find(i) != s.end() and s1.find(i) == s1.end())
                cnt1++;
            else if (s1.find(i) != s1.end() and s.find(i) == s.end())
                cnt1++;
        }
        cnt1 /= 2;
        // cout << cnt << endl;
        ...
        {
            if (i % 2)
                sum += (C(cnt, i) * fact[cnt + cnt1 - i]) % mod;
            else
                sum -= (C(cnt, i) * fact[cnt + cnt1 - i]) % mod;
            sum %= mod;
            if (sum < 0)
        ...
        if (cnt + cnt1)
        {
            ...
            f = 0;
        cout << ((f - sum) % mod + mod) % mod << endl;
        // cout << countDer(cnt) % mod << endl;
    }

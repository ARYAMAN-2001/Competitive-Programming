...
int main() {
    ...
    multiset<ll> s;
    ll cnt = 0, ans = 0, sum = 0, mans = val[n];
    for (i = n - 1; i >= 0; i--)
    {
        if (t[i] == 2)
        {
            ...
            if (y[i] < 0)
            {
                if (s.size() < max(0ll, k - cnt))
                {
                    sum += abs(y[i]);
                    s.insert(abs(y[i]));
                }
                else if (s.size() and s.size() == max(0ll, k - cnt))
                {
                    ll c = *s.begin();
                    if (c < abs(y[i]))
                    {
        ...
        else
        {
            if (k == cnt)
                break;
            ll ans1 = val[i + 1] + ans + sum;
            mans = max(mans, ans1);
            if (s.size() and s.size() == max(0ll, k - cnt))
            {
                ll c = *s.begin();
                sum -= c;
                s.erase(s.begin());

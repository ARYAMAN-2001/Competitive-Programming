...
ll query(ll v, ll tl, ll tr, ll l, ll r)
{
    ...
    ans = max(ans, query(2 * v + 1, tm + 1, tr, l, r));
    return ans;
}
// vll dx = {1, -1, 0, 0};
// vll dy = {0, 0, 1, -1};
...
int main() {
    ...
    while (t--)
    {
        ...
        ll n;
        cin >> n;
        ll a[n], i, sum = 0, res = 0, d = 1000000;
        vll v;
        fo(i, 0, n)
        {
            cin >> a[i];
        ...
        fo(i, 0, v.size())
        {
            ...
            while (st.empty() == false and st.top() / d < v[i])
            {
                ll idx1 = st.top() % d;
                ll val1 = st.top() / d;
                // cout << idx1 << " " << i << endl;
                ll max1 = query(1, 0, n - 1, idx1 + 1, i - 1);
                // cout << max1 << endl;
                if (max1 > psum[idx1])
                {
                    res = 1;
        ...
        while (!st.empty())
        {
            ll idx1 = st.top() % d;
            ll val1 = st.top() / d;
            ll max1 = query(1, 0, n - 1, idx1 + 1, n - 1);
            if (max1 > psum[idx1])
            {
                res = 1;
                break;
